...
#  URL: https://github.com/fmorisan/i3-backlight
#
#
...
notify_backlight() {
    ...
    local increase_backlight=$1
    local decrease_backlight=$1
    if $increase_backlight
    then
      local icon="~/bin/custom-icons/weather-clear-symbolic.symbolic-white.png"
    else
      local icon="~/bin/custom-icons/weather-clear-night-symbolic.symbolic-white.png"
    fi
...
get_progress_bar() {
    ...
    then
      local progress=$(($max_percent / $divisor))
    else
      local progress=$(($percent / $divisor))
    fi
...
if ${opt_notification}; then
    notify_backlight $opt_increase_backlight $opt_decrease_backlight
fi
